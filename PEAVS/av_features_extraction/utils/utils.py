...
def probe_audio_codec_with_ffmpeg(video_path: str) -> str:
    """Probe the video file with ffmpeg and return the audio codec."""
    cmd = [which_ffmpeg(), '-hide_banner', '-loglevel', 'panic', '-y', '-i', video_path]

    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    output = result.stderr

    # Look for a line that has 'Audio' in it. A typical line looks like:
    # Stream #0:1(und): Audio: mp3 (mp4a / 0x6134706D), 16000 Hz, stereo, fltp, 48 kb/s (default)
    for line in output.split('\n'):
        if 'Audio:' in line:
            parts = line.split(':')
            if len(parts) > 2:
                codec = parts[2].split(',')[0].strip()
                return codec
...
def extract_wav_from_mp4(video_path: str, tmp_path: str) -> str:
    ...
    audio_codec = probe_audio_codec_with_ffmpeg(video_path)

    if audio_codec == "aac":
        mp4_to_acc = f'{which_ffmpeg()} -hide_banner -loglevel panic -y -i {video_path} -acodec copy {audio_aac_path}'
    else:
        # Encode to aac for other audio codecs
        mp4_to_acc = f'{which_ffmpeg()} -hide_banner -loglevel panic -y -i {video_path} -c:a aac -b:a 128k {audio_aac_path}'

    # Continue with the conversion to .wav
